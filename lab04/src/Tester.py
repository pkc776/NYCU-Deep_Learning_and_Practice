...
class Test_model(VAE_Model):
    ...
    def forward(self, curr_frame, prev_frame, curr_label):
        ...
        gen_frame = self.Generator.forward(fusion)    
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        label = label.permute(1, 0, 2, 3, 4) # (seq, B, C, H, W)
        ...
        prev_frame = img[0]  # [B, C, H, W]
        device = self.args.device

        # Generate 629 frames (first frame is given)
        for t in range(1, 630):
            curr_label = label[t].to(device)  # [B, C, H, W]

            # Encode previous frame and label
            prev_feat = self.frame_transformation(prev_frame.to(device))
            label_feat = self.label_transformation(curr_label)

            # Sample z from standard normal for test
            z = torch.randn(prev_feat.size(0), self.args.N_dim, prev_feat.size(2), prev_feat.size(3), device=device)
            # print("prev_feat shape:", prev_feat.shape)
            # print("label_feat shape:", label_feat.shape)
            # print("z shape:", z.shape)
            # Decoder fusion and frame generation
            fusion = self.Decoder_Fusion.forward(prev_feat, label_feat, z)
            gen_frame = self.Generator.forward(fusion)
            gen_frame = torch.clamp(gen_frame, 0, 1)
            gen_frame = torch.nan_to_num(gen_frame, nan=0.0, posinf=1.0, neginf=0.0)
            decoded_frame_list.append(gen_frame.cpu())
            label_list.append(curr_label.cpu())
            prev_frame = gen_frame  # Use generated frame for next step
    ...
    def val_dataloader(self):
        ...
        print(f"Dataset root: {self.args.DR}")
        dataset = Dataset_Dance(root=self.args.DR, transform=transform, video_len=self.val_vi_len)  
        print(f"Validation dataset length: {len(dataset)}")  # Add this line
